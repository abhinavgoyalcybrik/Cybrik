...
from .models import Lead, TelephonyConfig, Tenant

logger = logging.getLogger(__name__)

# Smartflo API Configuration
SMARTFLO_API_BASE = getattr(settings, 'SMARTFLO_API_BASE', 'https://api-smartflo.tatateleservices.com')

def get_smartflo_credentials(tenant=None):
    """
    Retrieve Smartflo credentials for a tenant, falling back to legacy settings.
    Returns (api_key, caller_id, agent_id)
    """
    api_key = None
    caller_id = None
    agent_id = None
    ...
    if tenant:
        config = TelephonyConfig.objects.filter(tenant=tenant, provider='smartflo').first()
        if config:
            # In Phase 1, we stored the main API key. 
            # If VoiceBot key is needed, we ideally store it. 
            # For now, we return the decrypted key.
            api_key = config.get_api_key()
            
    # Fallback to legacy settings if not found in DB
    if not api_key:
        # Prefer VoiceBot key for API calls if available
        api_key = getattr(settings, 'SMARTFLO_VOICEBOT_API_KEY', '') or getattr(settings, 'SMARTFLO_API_KEY', '')
        
    caller_id = getattr(settings, 'SMARTFLO_CALLER_ID', '')
    agent_id = getattr(settings, 'SMARTFLO_AGENT_ID', '')
    
    return api_key, caller_id, agent_id
...
def initiate_ai_call(request):
    ...
    # Try to resolve tenant (from user or default)
    tenant = None
    if request.user and request.user.is_authenticated and hasattr(request.user, 'profile') and request.user.profile.tenant:
        tenant = request.user.profile.tenant
    
    # If no tenant found for user, try finding 'cybric' default tenant for backward compatibility/dev
    if not tenant:
        tenant = Tenant.objects.filter(slug='cybric').first()

    api_key, _, _ = get_smartflo_credentials(tenant)

    if not api_key:
        return Response(
            {'error': 'Smartflo API not configured. Please set SMARTFLO_API_KEY in .env or Configure Tenant'},
...
def initiate_smartflo_call(destination_number: str, websocket_url: str = None, custom_params: dict = None, tenant=None):
    """
    Call Smartflo Click to Call Support API to initiate outbound Voice Bot call.
    """
    
    # Smartflo Click to Call Support API endpoint
    # Documentation: https://api-smartflo.tatateleservices.com/v1/click_to_call_support
    # TESTED WORKING on 2026-01-10 with key d760f...
    url = f"{SMARTFLO_API_BASE}/v1/click_to_call_support"
    
    # Get credentials
    api_key, caller_id, agent_id = get_smartflo_credentials(tenant)
    
    headers = {
        'Content-Type': 'application/json'
        # Note: Support API expects key in BODY, not Authorization header
    }
    ...
    payload = {
        ...
        'caller_id': caller_id,
        'agent_extension': agent_id,
    }
    
    # Add optional parameters if needed
    if custom_params:
        # Custom fields can be passed for tracking via webhooks
        # Support API uses custom_field_1, custom_field_2, etc.
        for i, (key, value) in enumerate(list(custom_params.items())[:5]):
...
def end_call(request):
    ...
    try:
        # Resolve api key (assuming default tenant for now or passing user's tenant if we knew it)
        # Ideally we store tenant_id in CallRecord and look it up by call_sid, but we don't have CallRecord lookup here yet.
