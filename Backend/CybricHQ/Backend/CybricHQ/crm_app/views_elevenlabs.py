...
def elevenlabs_postcall(request):
    ...
    if has_messages:
        ...
        # === Send post-call WhatsApp message ===
        try:
            from .services.post_call_messaging import send_postcall_whatsapp
            whatsapp_result = send_postcall_whatsapp(call_record)
            if whatsapp_result.get('success'):
                logger.info("Post-call WhatsApp sent for CR %s to %s", call_record.id, whatsapp_result.get('phone'))
            else:
                logger.warning("Post-call WhatsApp failed for CR %s: %s", call_record.id, whatsapp_result.get('error'))
        except Exception:
            logger.exception("Failed to send post-call WhatsApp for CR %s", call_record.id)
