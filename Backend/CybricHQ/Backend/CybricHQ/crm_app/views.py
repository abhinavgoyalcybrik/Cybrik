...
class UniversityViewSet(TenantQuerySetMixin, viewsets.ModelViewSet):
...
# ============================================================================
# APPLICATION DOCUMENT VIEWSET
# ============================================================================

class ApplicationDocumentViewSet(viewsets.ModelViewSet):
    """CRUD for Application Documents."""
    queryset = ApplicationDocument.objects.all().order_by('-created_at')
...
class ApplicationViewSet(TenantQuerySetMixin, viewsets.ModelViewSet):
    ...
    def change_stage(self, request, pk=None):
        ...
        if new_stage not in valid_stages:
            return Response({'error': f'Invalid stage. Valid stages: {valid_stages}'}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def get_stages(self, request):
        """Get all available stages with counts."""
        qs = self.get_queryset()
        stage_counts = {}
        for stage_code, stage_name in Application.STAGE_CHOICES:
            stage_counts[stage_code] = {
                'code': stage_code,
                'name': stage_name,
                'count': qs.filter(stage=stage_code).count()
            }
        return Response(stage_counts)
    
    @action(detail=False, methods=['get'], url_path='kanban')
    def kanban_view(self, request):
        """Get applications grouped by stage for kanban board."""
        qs = self.get_queryset()
        result = {}
        for stage_code, stage_name in Application.STAGE_CHOICES:
            apps = qs.filter(stage=stage_code)
            result[stage_code] = {
                'name': stage_name,
                'applications': ApplicationListSerializer(apps, many=True).data
            }
...
class StaffViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        """
        Filter users based on tenant isolation:
        - Superusers (no tenant context): See only system-level staff (users with no tenant)
        - Tenant users: See only staff from their tenant
        """
        ...
        # Fallback: If middleware didn't set tenant, try getting it from user's profile
        if not tenant and user.is_authenticated:
            try:
                ...
                # Try querying PUBLIC schema for profile
                try:
        ...
        # 1. If there's a tenant context, show only users from that tenant
        if tenant:
            return queryset.filter(profile__tenant=tenant)
        
        # 2. Superusers WITHOUT tenant context = System-level view
        # Show only users who have NO tenant (system admins/staff)
        if user.is_superuser:
            # Show users who either have no profile OR have profile with tenant=None
            return queryset.filter(
                Q(profile__isnull=True) | Q(profile__tenant__isnull=True)
            )
            
        # 3. If no tenant context and not superuser, return empty (secure default)
        return queryset.none()

    def perform_create(self, serializer):
        role_name = serializer.validated_data.pop("role", "counsellor")
        ...
        # Create UserProfile with Tenant link
        tenant = getattr(self.request, 'tenant', None)
        from .models import Role, UserProfile
        
        # Find or create Role object (RBAC model, distinct from Group)
        rbac_role = Role.objects.filter(name__iexact=role_name).first()
        ...
        logger.info(f"Created staff user {user.username} for tenant {tenant}")

    def perform_update(self, serializer):
        role_name = serializer.validated_data.pop("role", None)
        # Check raw data if not in validated_data
        if not role_name and "role" in self.request.data:
            role_name = self.request.data["role"]
