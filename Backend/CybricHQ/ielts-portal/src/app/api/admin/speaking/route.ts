...
const DATA_PATH = path.join(process.cwd(), 'public', 'data', 'speaking_tests.json');
...
export async function POST(request: NextRequest) {
    try {
        ...
        // Update metadata
        parsed.metadata.totalTests = parsed.tests.length + 1;
...
export async function DELETE(request: NextRequest) {
    try {
        ...
        parsed.metadata.totalTests = parsed.tests.length;
