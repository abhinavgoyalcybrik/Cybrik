...
type Step = 'verify' | 'upload' | 'success' | 'error';

export default function MagicUploadPage({ params }: { params: Promise<{ tokenId: string }> }) {
    ...
    // On Load: Validate token and auto-send OTP
    useEffect(() => {
        const init = async () => {
            setLoading(true);
            try {
                // 1. Check Token Info
                const infoRes = await fetch(`${API_BASE}/api/upload-portal/${tokenId}/info/`);
                if (!infoRes.ok) throw new Error("Invalid or expired link");
                ...
                // 2. Request OTP
                const otpRes = await fetch(`${API_BASE}/api/upload-portal/${tokenId}/request-otp/`, { method: 'POST' });
                if (!otpRes.ok) throw new Error("Failed to send verification code");
                const otpData = await otpRes.json();

                setEmailMasked(otpData.email_masked || 'your email');
                setStep('verify');
        ...
        return (
            <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center p-4">
                <div className="bg-white p-8 rounded-xl shadow-lg max-w-md w-full text-center">
                    ...
                    <a href="/upload" className="block w-full bg-blue-600 text-white py-2 rounded-lg hover:bg-blue-700 transition">
                        Go to Main Portal
                    </a>
                </div>
            </div>
            ...
            <div className="w-full max-w-md bg-white rounded-xl shadow-lg overflow-hidden">
                ...
                {step !== 'success' && (
                    <div className="flex border-b">
                        <div className={`flex-1 h-1 ${step === 'verify' ? 'bg-blue-600' : 'bg-blue-200'}`} />
                ...
                <div className="p-8">
                    ...
                    {/* Step: Loading state during init */}
                    {loading && step === 'verify' && !emailMasked && (
                        <div className="text-center py-10">
                            <Loader2 className="animate-spin text-blue-600 h-8 w-8 mx-auto mb-4" />
                            <p className="text-gray-500">Verifying security link...</p>
                        </div>
                    )}

                    {/* Step 2: Verify OTP */}
                    {step === 'verify' && emailMasked && (
                    ...
                    {step === 'success' && (
                        <div className="text-center py-8">
                            ...
                            <p className="text-gray-500 mb-8">Your document has been securely submitted.</p>
